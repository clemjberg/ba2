...
def main() -> None:
    ensure_dirs()
    df = read_many("data/raw/server/ingestion/raw_ingestion_*.csv")
    if df.empty:
        print("No ingestion files found.")
        return
    ...
    cleaned = trim_warmup_shutdown(df, "t1_server_received_ms")
    cleaned = cleaned[cleaned["latency_ms"].notna()]
    cleaned = cleaned[cleaned["latency_ms"] >= 0]

    save(cleaned, "data/cleaned/ingestion_cleaned.csv")
    ...
    summaries = []
    for metric in ["latency_ms", "jitter_ms"]:
    ...
    if summaries:
        ...
        save(summary, "results/tables/ingestion_summary.csv")

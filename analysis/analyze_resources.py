...
def main() -> None:
    ensure_dirs()
    df = read_many("data/raw/server/resources/resources_server_process_*.csv")
    if df.empty:
        print("No resource files found.")
        return

    numeric_cols = [
        "timestamp_ms", "scenario_clients", "hz", "cpu_percent_process",
        "ram_mb_process", "go_goroutines", "active_ingest_connections",
        "active_dashboard_connections", "received_messages_total"
    ]
    ...
    save(cleaned, "data/cleaned/resources_cleaned.csv")
    ...
    summaries = []
    for metric in ["cpu_percent_process", "ram_mb_process", "go_goroutines"]:
    ...
    if summaries:
        ...
        save(summary, "results/tables/resources_summary.csv")
